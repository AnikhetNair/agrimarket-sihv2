...
export const FarmerDashboardPage: React.FC = () => {
  ...
  const getDecisionActionText = () => {
    if (decision.action === 'HOLD') {
      return t('farmer.holdAction', 'HOLD FOR +7 DAYS');
    }
    if (decision.action === 'SELL_NOW') {
      return t('farmer.sellAction', 'SELL NOW AT APMC');
    }
    return t('farmer.partialAction', 'PARTIAL SALE / BUFFER');
  };

  const getTranslatedReasons = () => {
    if (decision.action === 'HOLD') {
      return [
        t('farmer.holdReason1', 'Current Mandi rate (₹2,850/Q) is below 7-day forecast range (₹3,000–3,150/Q).'),
        t('farmer.holdReason2', 'Anticipated price gain of ₹4,500 exceeds cold-storage & holding cost of ₹1,350.'),
      ];
    }
    if (decision.action === 'SELL_NOW') {
      return [
        t('farmer.sellReason1', 'Immediate sale locks in ₹91,500 with zero transit loss.'),
        t('farmer.sellReason2', 'Drying supply forecast indicates downside risk over 48 hours.'),
      ];
    }
    return [t('farmer.partialReason1', 'Sell 50% immediately to ensure liquidity; hold balance for predicted price peak.')];
  };
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-2xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-[#0d0a0b] tracking-tight">
            {user?.name || 'Ramesh Patil'} — {t('farmer.operationsDesk', 'Farmer Operations Desk')}
          </h1>
          <p className="text-xs text-[#454955] mt-0.5 flex items-center space-x-2">
            ...
            <span className="text-[#386641] font-medium">
              {t('farmer.memberOf', 'Member: Sahyadri Farmers Producer Co.')}
            </span>
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2.5">
          ...
          >
            ...
            <span>{t('farmer.listProduceLot', 'List Produce Lot')}</span>
          </Link>
          <Link
            to="/farmer/market"
          ...
          >
            ...
            <span>{t('farmer.marketForecasts', 'Market Forecasts')}</span>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-[#454955]">
                  {t('farmer.apmcBenchmark', 'APMC Benchmark • Nashik Primary Hub')}
                </span>
              </div>
              <span className="text-[11px] text-[#454955]">
                {t('farmer.arrivalsToday', 'Arrivals Today')}: 850 {t('farmer.quintals', 'Quintals')}
              </span>
            </div>
            ...
            <div className="mt-4 flex items-baseline justify-between">
              <div>
                <span className="text-xs font-medium text-[#454955]">
                  {t('farmer.commodity', 'Commodity')}: Carrot (Kuroda Premium)
                </span>
                <div className="text-3xl font-extrabold text-[#0d0a0b] tracking-tight mt-0.5">
                  ₹{modalPrice.toLocaleString('en-IN')}{' '}
                  <span className="text-sm font-normal text-[#454955]">{t('farmer.perPerQuintal', '/ per Quintal')}</span>
                </div>
              </div>
              <div className="text-right">
                <span className="inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[#6A994E]/15 text-[#386641] border border-[#6A994E]/30">
                  ...
                  <span className="text-[10px] font-normal text-[#386641]">{t('farmer.vs7DayAvg', 'vs 7-day avg')}</span>
                </span>
                <div className="text-[11px] text-[#454955] mt-1">
                  {t('farmer.modalRange', 'Expected daily modal range')}: ₹2,750 - ₹2,950
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-2xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-[#454955]">
                {t('farmer.optimalStrategy', 'Optimal Strategy Engine')}
              </span>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 bg-[#F2E8CF] rounded-lg p-3 border border-slate-200 text-xs">
              <div>
                <span className="text-[#454955] text-[11px] font-medium">
                  {t('farmer.expected7DayRange', 'Expected 7-Day Market Range')}
                </span>
              ...
              <div>
                <span className="text-[#454955] text-[11px] font-medium">
                  {t('farmer.projectedNetGain', 'Projected Net Extra Realisation')}
                </span>
            ...
            <div className="mt-4">
              <h4 className="text-xs font-semibold text-[#0d0a0b] mb-2">
                {t('farmer.whyStrategy', 'Why This Strategy? (Deterministic Rule Explainability)')}
              </h4>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
              <span className="text-[11px] text-[#454955]">
                {t('farmer.calculatedNotice', 'Calculated deterministically using verified APMC modal & mandi spreads')}
              </span>
              <Link
                to="/farmer/market"
                className="inline-flex items-center space-x-1 text-xs font-semibold text-[#386641] hover:text-[#2d5535]"
              >
                <span>{t('farmer.exploreBenchmarks', 'Explore Price Forecasts & Model Benchmarks')}</span>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-2xs">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <span className="text-xs font-bold uppercase tracking-wider text-[#454955]">
                {t('farmer.currentLotInNeg', 'Current Lot in Negotiation')}
              </span>
            ...
            {goldenLot ? (
              <div className="mt-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 bg-[#F2E8CF] rounded-lg p-2.5 border border-slate-200 text-xs">
                  <div>
                    <span className="text-[10px] text-[#454955] font-medium">{t('farmer.askingRate', 'Asking Rate')}</span>
                    <div className="font-semibold text-[#0d0a0b]">₹{goldenLot.asking_price}/Q</div>
                  </div>
                  <div>
                    <span className="text-[10px] text-[#454955] font-medium">{t('farmer.floorRate', 'Floor Rate')}</span>
                ...
                <div className="p-3 bg-[#6A994E]/10 rounded-lg border border-[#6A994E]/30">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-[#0d0a0b] flex items-center space-x-1">
                      ...
                      <span>{t('farmer.verifiedOffer', 'Verified Buyer Offer')} (94%)</span>
                    </span>
                ...
                <div className="pt-2">
                  ...
                  >
                    ...
                    <span>{t('farmer.dealsTitle', 'Deals & Negotiation')}</span>
              ...
              <div className="mt-4 text-center py-6 text-xs text-[#454955]">
                {t('farmer.noLotsFound', 'No active produce lots registered.')}
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-2xs">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold uppercase tracking-wider text-[#454955]">
                {t('farmer.activeDealsTitle', 'Active Trade Negotiations')} ({activeDeals.length})
              </span>
              <Link
                to="/farmer/deals"
